...
function test_start_fen_parse_equals_start_board() {
    let g1 = parse_fen(start_fen);
    let g2 = Game.start();
    return JSON.stringify(g1) == JSON.stringify(g2);
}

function test_unparse_parse_square() {
    for (let sq = 0; sq < 64; sq++) {
        let i = SQUARES[sq];
        if (i != parse_square(unparse_square(i))) {
            console.log(i, parse_square(unparse_square(i)));
...
// drawing tests
function test_draw_start_canvas() {
    var game = Game.start();
    var gui = new Gui(game);
}

// performance tests
function test_genmoves_start_1e5() {
    var game = Game.start();
    return timeit(() => game.gen_moves(), 1e5);
}

// enpassant tests
function test_enpassant_smoke() {
    var game = Game.start();
    
}

// perft test
// https://raw.githubusercontent.com/AndyGrant/Ethereal/master/src/perft/standard.epd
const STANDARD_EPD = `rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1 ;D1 20 ;D2 400 ;D3 8902 ;D4 197281 ;D5 4865609 ;D6 119060324
...
function test_standard_epd(max_nodes=1e6) {
    for (let epd of STANDARD_EPD.split("\n")) {
        let split = epd.split(";");
        let fen = split[0];
        for (let i = 1; i < split.length; i++) {
            let depth_split = split[i].split(" ");
            let depth = parseInt(depth_split[0].slice(1));
            let nodes = parseInt(depth_split[1]);
            if (nodes < max_nodes) {
                let result = parse_fen(fen).perft(depth);
                console.log(fen, depth, nodes, result);
                if (result != nodes) {
                    throw new Error("^Failed");
                }
            }
